...
const PmDashboard = () => {
  ...
  const activeProjects = Array.isArray(projects) ? projects.filter((p) => p.is_active).length : 0;
  const teamMembers = Array.isArray(projects) ? new Set(projects.flatMap((p) => (p.members || []).map((m) => m.id))).size : 0;
